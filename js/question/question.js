...
var startlist = [
	...
	'我重新登录后，怎么任何信息都不显示了',
	'为什么实名不通过',
	'EOTC1111113030订单没有收到',
	'邮箱收不到验证码',
	'为什么我的会员系统认证信息和收款信息都提交成功了，但交易所里面的只有认证信息，收款信息连姓名也没有，填写好再怎么提交都不行',
	'冲币未到账？',
	'上周我已在EOTC上成功交易了一次可是我的空投到现在还没有被释放？',
	'您好！请问邮箱可以更改吗？，我的邮箱显示在异地登入？',
	'EOTC2.0几号上线？'
];
...
var questionObjList = [
	...
	},
	{
		title: '提币？',
		content:
			'<div>尊敬的用户：您好！你已经提交提币地址，请把提币地址发到eotcotc@gmail.com 格式如下<br />姓名：<br />UID： <br />提币地址： <br />注意：请使用与UID相对应的邮箱发。   谢谢您的配合！</div>'
	},
	{
		title: '如何释放EOTC？',
		content: '<div>上交易所卖一次USDT，排队释放</div>'
	...
	},
	{
		title: '邮箱收不到验证码？',
		content:
			'<div>一、首先检查你的邮箱是否有效，发个邮件确认有效邮箱<br />二、现在用QQ邮箱收验证码默认是到垃圾邮箱里面，所以请到垃圾邮箱里找，因为注册的人太多，系统发送频率太高，被识别为垃圾邮件了，进到垃圾邮箱，把发送方，设置为（这不是垃圾邮件）下一次就可以正常收到邮件提示了<br />三、只发一次，只点击一次，连续点击发送验证码多次属操作有误，间隔10分钟属正常操作<br />四、清空手机缓存，也许手机储存空间已经没有了<br />五、检查手机网络，也许手机根本没网络，保持手机网络畅通<br />六、系统问题，可能此时此刻该地区或区域已经被屏蔽，请联系客服邮箱：coin@eotc.me，或电报官方群反应情况，工作时间内客服两小时内给回复。</div>'
	},
	{
...
function goPage(pno, psize) {
	...
	if (num > 0) {
		if (num > 1) {
			...
			if (currentPage > 1) {
				tempStr += '<div class="preBtn"><a href="#" onClick="goPage(' + (currentPage - 1) + ',' + psize + ')"><img src="../../img/leftPageIcon.png" /></a></div>';
				for (var j = 1; j <= totalPage; j++) {
					tempStr += '<div class="numBtn"><a href="#" onClick="goPage(' + j + ',' + psize + ')">' + j + '</a></div>';
				}
			} else {
			...
			if (currentPage < totalPage) {
				tempStr += '<div class="nextBtn"><a href="#" onClick="goPage(' + (currentPage + 1) + ',' + psize + ')"><img src="../../img/rightPageIcon.png" /></a></div>';
				for (var j = 1; j <= totalPage; j++) {}
			} else {
